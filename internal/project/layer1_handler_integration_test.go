...
func layer1HTTPHarness(t *testing.T, db *sql.DB) (http.Handler, *config.Config) {
	...
	mux.Handle("GET /v1/orgs/{orgID}/projects", orgScoped("project.list", http.HandlerFunc(projectHandler.ListProjectsByOrganizationID)))
	mux.Handle("GET /v1/orgs/{orgID}/projects/{projectID}", orgScoped("project.view", http.HandlerFunc(projectHandler.GetProjectByID)))
	mux.Handle("PATCH /v1/orgs/{orgID}/projects/{projectID}", orgScoped("project.update", http.HandlerFunc(projectHandler.Update)))
